        ...
        {tvShows.length === 0 ? (
          ...
          <div className="space-y-6 text-left">
            <div className="flex items-center gap-3 border-l-4 border-red-500 pl-3">
              <h3 className="text-xl font-bold tracking-tight text-white uppercase">Search Results ({filteredShows.length})</h3>
            </div>
            ...
            {filteredShows.length === 0 ? (
              <div className="rounded-2xl border border-white/5 bg-white/5 p-12 text-center max-w-md mx-auto">
                ...
                <p className="text-gray-400 font-light text-xs">
                  We couldn't find any TV shows matching "{searchQuery}".
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredShows.map((show) => (
                  <div
                    key={show.id}
                    onClick={() => navigate(`/movie/tmdb-tv-${show.tmdbId || show.id}`)}
